...
- Funciones fuera del plan (pedido del usuario, siempre): nunca esconderlas; mostrarlas con el plan que las desbloquea y un link para contratarlo (`ProLockedCard`, `PlanLockNote`, `PlanPill` en `src/components/dashboard/pro-locked-card.tsx`). Es progresivo: sólo se ofrecen planes más altos que el actual (en el Plan IA no se ofrece nada). Si hay datos (p. ej. cuántos productos con stock bajo), mostrar el número e invitar a desbloquear el detalle.
