...
- Para ubicar código y relaciones sin leer todo: mapa de Graphify en `graphify-out/` (`GRAPH_REPORT.md`; `graphify query "…"`, `graphify explain "X"`, `graphify path "A" "B"`). Se regenera solo al empezar la sesión (hook en `.claude/settings.json`); después de cambios grandes, `graphify update .` (sin costo de API).
