...
export function LiveClient({
  ...
  const yesterday = data.yesterday_same_time.total;
  const deltaPct = yesterday > 0 ? ((data.today.total - yesterday) / yesterday) * 100 : null;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label="Cajas abiertas" value={String(openCount)}>
          <span className="text-muted-foreground">de {data.members.length} personas del equipo</span>
        </Stat>
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-border">
            {data.members.map((m) => (
              <MemberRow key={m.user_id} member={m} label={labelById.get(m.user_id)!} now={now} />
            ))}
          </div>
        </CardContent>
      </Card>
...
function MemberRow({ member, label, now }: { member: LiveMember; label: string; now: number }) {
